...
def init_scores_db():
    ...
    for valor, puntaje in hp_scores:
        cur.execute(
            "INSERT OR IGNORE INTO scores (apartado, valor, puntaje) VALUES (?, ?, ?)",
            ("hp", valor, puntaje),
        )
    ...
    susp_tras_scores = [
        ("doble_amortiguador", 4),
        ("monoamortiguador_basico", 7),
    ...
    vel_crucero_scores = [("60-80", 4), ("80-100", 7), ("100-120", 9), ("120+", 10)]
    ...
    for marca, puntaje in marcas_confiabilidad:
        cur.execute(
            "INSERT OR IGNORE INTO marca_confiabilidad (marca, puntaje) VALUES (?, ?)",
            (marca, puntaje),
        )
    ...
    for palabra, penalizacion in fallos_penalizaciones:
        cur.execute(
            "INSERT OR IGNORE INTO fallos_scores (palabra, penalizacion) VALUES (?, ?)",
    ...
    for respuesta in ["si", "no"]:
        for campo in [
            "parabrisas_ajustable",
            "control_crucero",
            "sliper_clutch",
            "suspension_largo_recorrido",
            "proteccion_motor",
            "tanque_grande",
            "maletas_laterales",
            "suspension_ajustable",
        ]:
            cur.execute(
                "INSERT OR IGNORE INTO valores_validos (campo, valor, categoria, ejemplo) VALUES (?, ?, ?, ?)",
    ...
    for campo, min_v, max_v, unidad, ejemplo in rangos:
        cur.execute(
            "INSERT OR IGNORE INTO rangos_validos (campo, min_valor, max_valor, unidad, ejemplo) VALUES (?, ?, ?, ?, ?)",
