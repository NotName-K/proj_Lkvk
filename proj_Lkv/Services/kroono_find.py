...
class KronoFind:
    ...
    def _calcular_y_ordenar(self, motos_lista):
        ...
        precios = [m.info.precio for m in motos_lista if m.info.precio and m.info.precio > 0]
    ...
    def mostrar_resultados(self, resultados, titulo):
        ...
        if not resultados:
            ...
            print(f"{'Pos':<4} {'Moto':<40} {'Precio':>14} {'KronoPrecio':>12}  Clasificación")
            print("-"*90)
            for i, res in enumerate(resultados[:10], 1):  # solo las 10 mejores
                m = res['moto']
                s = res['scores']
                precio_texto = f"${m.info.precio:,.0f}" if m.info.precio and m.info.precio > 0 else "Consultar"
                clasif = clasificar_krono(s['krono_precio'])
                nombre = f"{m.info.marca} {m.info.modelo} ({m.info.año})"
                print(f"{i:<4} {nombre:<40} {precio_texto:>14} {s['krono_precio']:>10.1f}    {clasif}")
    ...
    def filtrar_por_presupuesto(self):
        try:
            ...
            print("Valor inválido"); input(); return
    ...
    def filtrar_por_tipo(self):
        tipos = ['naked', 'sport', 'touring', 'adventure', 'scooter', 'street', 'doble pps', 'electric', 'motocarro']
        print("\nTipos de moto:")
        ...
        resultados = self._calcular_y_ordenar(motos)
        self.mostrar_resultados(resultados, f"MEJORES MOTOS TIPO: {tipo_elegido.upper()}")
    ...
    def filtrar_por_cilindraje(self):
        ...
        try:
            op = input("\nElige: ")
            limites = { '1': (0,200), '2': (201,500), '3': (501,800), '4': (801,9999) }[op]
        except:
            print("Opción inválida"); input(); return
        ...
        resultados = self._calcular_y_ordenar(motos)
        self.mostrar_resultados(resultados, f"MEJORES MOTOS {limites[0]}–{limites[1]} cc")
    ...
    def filtrar_por_uso(self):
        ...
        resultados = self._calcular_y_ordenar(motos)
        titulo = { '1':'CIUDAD', '2':'VIAJES LARGOS', '3':'OFF-ROAD', '4':'TRABAJO' }[op]
    ...
    def mostrar_top_general(self):
        ...
        resultados = self._calcular_y_ordenar(motos)
        self.mostrar_resultados(resultados, "TOP 10 GENERAL – MEJORES MOTOS SEGÚN KRONOSCORE")