...
class Comparador:
    ...
    def ejecutar(self):
        ...
        while len(seleccionadas) < 4:
            ...
            for i, m in enumerate(motos_raw, 1):
                ...
                precio_texto = f"${precio_valor:,.0f}" if precio_valor and precio_valor > 0 else "Consultar"
    ...
    def mostrar_comparacion(self, motos):
        ...
        precios_validos = [m.info.precio for m in motos if m.info.precio and m.info.precio > 0]
        precio_prom = sum(precios_validos) / len(precios_validos) if precios_validos else 15000000

        resultados = self.krono.comparar_motos(motos, precio_prom)

        print(f"{'Pos':<4} {'Moto':<45} {'Calidad':<10} {'Precio':<10} {'Clasificación'}")
        print("-" * 100)
        for i, res in enumerate(resultados, 1):
            ...
            print(f"{i:<4} {nombre:<45} {s['krono_calidad']:<10.1f} {s['krono_precio']:<10.1f} {clasif}")
    ...
    def mostrar_radar_comparativo(self, moto1, moto2, scores1, scores2):
        ...
        ax.legend(loc='upper right', bbox_to_anchor=(1.3, 1.1), fontsize=12, frameon=True, fancybox=True, shadow=True)

        plt.title("KronoScore - Comparación Radar", size=20, weight='bold', color='#2c3e50', pad=40)
        plt.tight_layout()
        plt.show()