...
class KronoScore:
    ...
    def _score_confiabilidad(self, m):
        ...
        penalizacion_total = sum(abs(self.db_scores.get_penalizacion(f)) for f in fallos)
    ...
    def actualizar_promedios_mercado(self):
        ...
        try:
            ...
            for moto in motos:
                scores = self.calcular(moto, precio_promedio_categoria=precio_promedio_general)
            ...
            for tipo, lista in stats.items():
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (tipo, avg_precio, avg_calidad, avg_krono, len(lista), date.today()))
    ...
    def vs_mercado(self, moto, scores):
        ...
        try:
            ...
            if motos_del_tipo:
                avg_precio = sum(m['precio'] for m in motos_del_tipo) / len(motos_del_tipo)
                avg_calidad = sum(m['calidad'] for m in motos_del_tipo) / len(motos_del_tipo)
                avg_krono = sum(m['krono_precio'] for m in motos_del_tipo) / len(motos_del_tipo)

                cur.execute("""
                    INSERT OR REPLACE INTO promedios_mercado
                    (tipo, precio_promedio, krono_calidad_promedio, krono_precio_promedio, cantidad_motos, ultima_actualizacion)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (tipo_moto, avg_precio, avg_calidad, avg_krono, len(motos_del_tipo), date.today()))

        
            cur.execute("SELECT krono_precio_promedio FROM promedios_mercado WHERE tipo = ?", (tipo_moto,))
            row = cur.fetchone()
            conn.commit()
            conn.close()
